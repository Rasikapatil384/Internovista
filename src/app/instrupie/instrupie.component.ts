  ...
  @Component({
    selector: 'app-instrupie',
    templateUrl: './instrupie.component.html',
    styleUrl: './instrupie.component.css'
  })

  export class InstrupieComponent implements OnInit {
      ...
      this.pieService.getData().subscribe(
        (data: any) => {
          if (data && data.CompDepartment) {
            ...
            const compData = [
              data.InstruDepartment['Core'] || 0,
              data.InstruDepartment['FinTech'] || 0,
              data.InstruDepartment['Others'] || 0,
              data.InstruDepartment['Tech'] || 0
            ];
