...
@Component({
  selector: 'app-instrubar',
  templateUrl: './instrubar.component.html',
  styleUrl: './instrubar.component.css'
})

export class InstrubarComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.barservice.getData().subscribe(
      (data) => {
        ...
        if (data) {
          const Count = data.instru || 0;  // Extract 'Comp' from the response data

          // Set the 'Interns' value dynamically
          this.config.data.datasets[0].data = [Count];
          this.config.data.datasets[1].data = [60-Count];
