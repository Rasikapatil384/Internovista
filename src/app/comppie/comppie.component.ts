...
@Component({
  selector: 'app-comppie',
  templateUrl: './comppie.component.html',
  styleUrls: ['./comppie.component.css']
})
export class ComppieComponent implements OnInit {
    ...
    this.pieService.getData().subscribe(
      (data: any) => {
        if (data && data.CompDepartment) {
          ...
          const compData = [
            data.CompDepartment['Core'] || 0,
            data.CompDepartment['FinTech'] || 0,
            data.CompDepartment['Others'] || 0,
            data.CompDepartment['Tech'] || 0
          ];
