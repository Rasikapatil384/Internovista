  ...
  @Component({
    selector: 'app-itpie',
    templateUrl: './itpie.component.html',
    styleUrl: './itpie.component.css'
  })

  export class ItpieComponent implements OnInit {
      ...
      this.pieService.getData().subscribe(
        (data: any) => {
          if (data && data.CompDepartment) {
            ...
            const compData = [
              data.ItDepartment['Core'] || 0,
              data.ItDepartment['FinTech'] || 0,
              data.ItDepartment['Others'] || 0,
              data.ItDepartment['Tech'] || 0
            ];
