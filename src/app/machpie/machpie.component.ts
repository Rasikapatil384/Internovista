  ...
  @Component({
    selector: 'app-machpie',
    templateUrl: './machpie.component.html',
    styleUrl: './machpie.component.css'
  })

  export class MachpieComponent implements OnInit {
      ...
      this.pieService.getData().subscribe(
        (data: any) => {
          if (data && data.CompDepartment) {
            ...
            const compData = [
              data.MechDepartment['Core'] || 0,
              data.MechDepartment['FinTech'] || 0,
              data.MechDepartment['Others'] || 0,
              data.MechDepartment['Tech'] || 0
            ];
